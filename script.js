...
`;
                ...
                let topInfoItems = [];
                topInfoItems.push(`<div><span>Contact</span></div>`);
                if (data.city || data.country) topInfoItems.push(`<div><span>Address</span> ${escapeHTML([data.city, data.country].filter(Boolean).join(', '))}</div>`);
                if (data.phone) topInfoItems.push(`<div><span>Phone</span> ${escapeHTML(data.phone)}</div>`);
                if (data.email) topInfoItems.push(`<div><span>Email</span> ${escapeHTML(data.email)}</div>`);
                if (additionalInfo.website) topInfoItems.push(`<div><span>Website</span> ${escapeHTML(additionalInfo.website)}</div>`);
                if (additionalInfo.linkedin) topInfoItems.push(`<div><span>LinkedIn</span> ${escapeHTML(additionalInfo.linkedin)}</div>`);
                ...
                if (resumeData.experienceType !== 'fresher') {
                    if (workExpArray.length > 0) {
                        ...
                        for (let i = 0; i < workExpArray.length; i++) {
                            const exp = workExpArray[i];
                            if (!exp.company.trim()) continue;
                            const durationText = exp.current ? `NOW - ${escapeHTML(exp.startYear)}` : `${escapeHTML(exp.startYear)} - ${escapeHTML(exp.endYear)}`;
                            t17Html += `
                                <div class="job">
                        ...
                        t17Html += `
                                <div class="edu">
                                    ...
                                    <div class="edu-right">
                                        ...
                                        ${edu.coursework ? `<br><span style="font-size:10px; color:#555;">Coursework: ${escapeHTML(edu.coursework)}</span>` : ''}
                                    </div>
                                </div>
                        `;
                    }
                        ...
                        t17Html += `
                                <div class="job">
                                    <div class="job-right">
                                        ...
                                        <div class="text" style="margin-top:2px;">${p.desc}</div>
                                    </div>
                                </div>
                        `;
                    }
                ...
                let pdText = [];
                if (additionalInfo.nationality) pdText.push(`<span>Nationality:</span> ${escapeHTML(additionalInfo.nationality)}`);
                if (additionalInfo.maritalStatus) pdText.push(`<span>Marital Status:</span> ${escapeHTML(additionalInfo.maritalStatus)}`);
                if (additionalInfo.visaStatus) pdText.push(`<span>Visa Status:</span> ${escapeHTML(additionalInfo.visaStatus)}`);
                if (additionalInfo.dob) pdText.push(`<span>DOB:</span> ${escapeHTML(additionalInfo.dob)}`);
                ...
                if (pdText.length > 0) {
                    t17Html += `
                            ...
                            <div class="section">
                                <div class="section-title">Personal Details</div>
                                <div class="top-info" style="margin-top: 6px;">
                                    ${pdText.map(t => `<div>${t}</div>`).join('')}
                                </div>
                            </div>
                    `;
                }
